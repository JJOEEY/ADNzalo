 ...
 *
 * Patches the Electron dev binary with ADNzalo's icon and version metadata.
...
async function main() {
  ...
  try {
    ...
    await rcedit(electronExePath, {
      icon: iconPath,
      'version-string': {
        ProductName:     pkg.build?.productName || 'ADNzalo',
        FileDescription: pkg.build?.productName || 'ADNzalo',
        CompanyName:     'ADN Capital',
      },
      'file-version':    pkg.version,
      'product-version': pkg.version,
    });

    console.log('[patch-icon] ✅ electron.exe patched with ADNzalo icon');
  } catch (err) {
