...
if (process.platform === 'win32') {
  app.setAppUserModelId(isDev ? `com.adncapital.adnzalo.dev.${Date.now()}` : 'com.adncapital.adnzalo');
}
...
//
// adnzalo://openChat?accountId=xxx&threadId=yyy&threadType=0&channel=zalo
...
function createWindow() {
  ...
  app.on('second-instance', (_event, argv) => {
    ...
    const deepLinkUrl = argv.find((arg: string) => arg.startsWith('adnzalo://'));
    if (deepLinkUrl) {
  ...
  ipcMain.on('app:setBadge', (_event, count: number) => {
    ...
    if (process.platform === 'darwin') {
      ...
      if (count > 0) {
        ...
        tray?.setToolTip(`ADNzalo - ${count} tin chưa đọc`);
      } else {
        if (currentIconIsDot) {
...
/**
 * Xử lý deep link URL từ custom protocol adnzalo://
 *
 * Định dạng:
 *   adnzalo://openChat?accountId=xxx&threadId=yyy&threadType=0&channel=zalo
 *
...
async function startupAllWorkspaces(): Promise<void> {
  ...
  for (const ws of localWorkspaces) {
    try {
      const dbPath = wsMgr.resolveDbPath(ws.dbPath || 'adnzalo-tool.db');
...
app.whenReady().then(async () => {
  ...
  // ── Register adnzalo:// as default protocol client ─────────────────────
  // Cho phép OS mở app khi click link adnzalo:// trong trình duyệt
  //
  ...
  if (app.isPackaged) {
    if (!app.isDefaultProtocolClient('adnzalo')) {
      app.setAsDefaultProtocolClient('adnzalo');
    }
  }
