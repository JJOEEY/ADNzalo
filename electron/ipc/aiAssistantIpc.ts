...
export function registerAIAssistantIpc(): void {
  ...
  ipcMain.handle('ai:suggest', async (_e, { assistantId, chatHistory, zaloId }: { assistantId: string; chatHistory: any[]; zaloId?: string }) => {
    try {
        ...
        try {
          ...
          if (ws?.bossUrl && ws?.token) {
            ...
            const res = await fetch(`${baseUrl}/api/command/ai/suggest`, {
              ...
              body: JSON.stringify({ assistantId, chatHistory, zaloId }),
      ...
      const suggestions = await AIAssistantService.getInstance().getSuggestions(assistantId, chatHistory, { zaloId });
  ...
  // ─── Direct chat ──────────────────────────────────────────────────────────
  ipcMain.handle('ai:chat', async (_e, { assistantId, messages, structured, maxTokens, zaloId }: { assistantId: string; messages: any[]; structured?: boolean; maxTokens?: number; zaloId?: string }) => {
    try {
      // Employee mode: proxy qua Boss REST API để Boss gọi AI
      if (isEmployeeMode()) {
        try {
          ...
          if (ws?.bossUrl && ws?.token) {
            ...
            const res = await fetch(`${baseUrl}/api/command/ai/chat`, {
              method: 'POST',
              headers: { 'Authorization': `Bearer ${ws.token}`, 'Content-Type': 'application/json' },
              body: JSON.stringify({ assistantId, messages, structured, maxTokens, zaloId }),
      ...
      Logger.info(`[AIAssistantIpc] chat: assistantId=${assistantId}, zaloId=${zaloId || 'none'}, messagesCount=${messages?.length}, structured=${!!structured}, maxTokens=${maxTokens ?? 'default'}`);
      const result = await AIAssistantService.getInstance().chat(assistantId, messages, !!structured, maxTokens, { zaloId });
