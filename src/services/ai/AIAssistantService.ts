...
class AIAssistantService {
   ...
   * @param senderName - Tên Zalo người gửi (động theo nick Huy/clone), thay cho prompt cố định Ngân
   */
  private async buildSystemPrompt(assistant: AIAssistant, forWorkflow: boolean = false, senderName?: string): Promise<string> {
    const parts: string[] = [];

    // 1. Custom system prompt - thay placeholder <Tên người gửi> bằng tên thực tế
    let systemPrompt = assistant.systemPrompt || '';
    if (senderName) {
      // Hỗ trợ các placeholder: {{senderName}}, <Tên người gửi>, Ngân
      systemPrompt = systemPrompt
        .replace(/\{\{senderName\}\}/g, senderName)
        .replace(/<Tên người gửi>/g, senderName)
        .replace(/\bNgân\b/g, senderName);
      // Nếu prompt gốc không có placeholder mà vẫn cố định Ngân, tự chèn dòng đầu
      if (!assistant.systemPrompt?.includes(senderName) && !assistant.systemPrompt?.includes('{{senderName}}')) {
        parts.push(`Bạn là ${senderName}, nhân viên tư vấn của ADN Capital. Xưng hô theo tên này (không cố định là Ngân).`);
      }
    }
...
5. KHÔNG BAO GIỜ trả về text thường. LUÔN LUÔN trả về JSON array như trên.`);
   ...
   */
  public async getSuggestions(assistantId: string, chatHistory: Array<{ role: string; content: string }>, opts?: { zaloId?: string }): Promise<string[]> {
    const assistant = this.getAssistant(assistantId) || this.ensureDefaultAssistant();
    if (!assistant || !assistant.enabled) return [];

    const senderName = this.getSenderNameForZaloId(opts?.zaloId);
    ...
    const persona = senderName ? `Bạn là ${senderName}, nhân viên ADN Capital. ` : '';
    const messages: ChatMessage[] = [
      {
        role: 'system',
        content: `${persona}Bạn là trợ lý gợi ý tin nhắn cho người bán hàng / chăm sóc khách hàng.
...
- Mỗi câu phải ngắn gọn (dưới 100 ký tự), tự nhiên như chat thật, xưng hô theo tên người gửi (${senderName || 'Huy'})
- Khi cần nhắc tên khách, giữ placeholder <Tên Zalo người nhận> để tái dùng
...
["Câu gợi ý 1","Câu gợi ý 2","Câu gợi ý 3","Câu gợi ý 4","Câu gợi ý 5"]`
  ...
  private getSenderNameForZaloId(zaloId?: string): string | undefined {
    if (!zaloId) return undefined;
    try {
      const db = DatabaseService.getInstance();
      const row = db.queryOne<any>(`SELECT full_name, display_name FROM accounts WHERE zalo_id = ?`, [zaloId]);
      const name = row?.full_name || row?.display_name || null;
      if (name) return String(name).split(' ').slice(-1).join(' ') || String(name);
    } catch {}
    return undefined;
  }

  private ensureDefaultAssistant(): AIAssistant | null {
    let def = this.getDefaultAssistant();
    if (def) {
      // Tự động cập nhật model cũ sang gpt-5.6-luna nếu đang dùng model cũ
      if (def.model !== 'gpt-5.6-luna' && (def.model === 'gpt-5.4-mini' || def.model === 'gpt-4o-mini' || def.platform !== 'openai')) {
        try {
          def.model = 'gpt-5.6-luna';
          def.platform = 'openai' as AIPlatform;
          // Giữ nguyên systemPrompt nếu đã có placeholder, nếu không thì cập nhật template động
          if (!def.systemPrompt.includes('{{senderName}}') && !def.systemPrompt.includes('<Tên người gửi>')) {
            def.systemPrompt = `Bạn là {{senderName}}, nhân viên tư vấn của ADN Capital (người chính là Huy, các nick còn lại là clone của Huy). Xưng hô theo tên này, gọi khách là anh/chị. Giọng tự nhiên như Zalo thật, ngắn gọn 2-4 câu, có quan điểm cá nhân. Khi viết kịch bản phải giữ placeholder <Xưng hô> <Tên Zalo người nhận> để tái dùng, và có thể chèn ảnh.`;
          }
          this.saveAssistant(def as any);
          Logger.info(`[AIAssistant] auto-migrated default assistant to openai/gpt-5.6-luna with dynamic sender`);
        } catch {}
      }
      return def;
    }
    // Tạo mới nếu chưa có
    try {
      const id = this.saveAssistant({
        name: 'ADN Assistant - Huy',
        platform: 'openai' as AIPlatform,
        apiKey: '',
        model: 'gpt-5.6-luna',
        systemPrompt: `Bạn là {{senderName}}, nhân viên tư vấn của ADN Capital (người chính là Huy, các nick còn lại là clone của Huy). Xưng hô theo tên này, gọi khách là anh/chị. Giọng tự nhiên như Zalo thật, ngắn gọn 2-4 câu, có quan điểm cá nhân. Khi viết kịch bản phải giữ placeholder <Xưng hô> <Tên Zalo người nhận> để tái dùng, và có thể chèn ảnh.`,
        maxTokens: 800,
        temperature: 0.92,
   ...
   */
  public async chat(assistantId: string, conversationMessages: Array<{ role: string; content: string }>, structured: boolean = false, maxTokensOverride?: number, opts?: { zaloId?: string; senderName?: string }): Promise<{ result: string; totalTokens: number; promptTokens: number; completionTokens: number }> {
    const assistant = this.getAssistant(assistantId) || this.ensureDefaultAssistant();
   ...
   */
  public async chatForWorkflow(assistantId: string, conversationMessages: Array<{ role: string; content: string }>, opts?: { zaloId?: string }): Promise<{ result: string; totalTokens: number; promptTokens: number; completionTokens: number }> {
    const assistant = this.getAssistant(assistantId) || this.ensureDefaultAssistant();
    if (!assistant) throw new Error('Trợ lý AI không tồn tại');
    if (!assistant.enabled) throw new Error('Trợ lý AI đã bị tắt');

    const senderName = this.getSenderNameForZaloId(opts?.zaloId);
    const systemPrompt = await this.buildSystemPrompt(assistant, true, senderName);
    const messages: ChatMessage[] = [
      { role: 'system', content: systemPrompt },
      ...conversationMessages.map(m => ({
        role: m.role as 'user' | 'assistant',
