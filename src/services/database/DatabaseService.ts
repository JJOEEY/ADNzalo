...
class DatabaseService {
    ...
    public async initialize(): Promise<void> {
        try {
            ...
            if (!resolvedViaWorkspace) {
                ...
                const configPath = path.join(userDataPath, 'adnzalo-config.json');
                ...
                this.dbPath = path.join(dbFolder, 'adnzalo-tool.db');
            }
    ...
    /**
     * Reinitialize DatabaseService từ path mới (đọc lại adnzalo-config.json).
    ...
    private createTables(): void {
        ...
        `);
        // Migration: đảm bảo default assistant dùng gpt-5.6-luna + prompt động theo tên nick
        try {
          const defRows = this.query<any>(`SELECT id, model, platform, system_prompt FROM ai_assistants WHERE is_default = 1 LIMIT 1`);
          if (defRows.length > 0) {
            const def = defRows[0];
            if (def.model !== 'gpt-5.6-luna' || def.platform !== 'openai' || !def.system_prompt.includes('{{senderName}}')) {
              this.run(`UPDATE ai_assistants SET model = 'gpt-5.6-luna', platform = 'openai', system_prompt = 'Bạn là {{senderName}}, nhân viên tư vấn của ADN Capital (người chính là Huy, các nick còn lại là clone của Huy). Xưng hô theo tên này, gọi khách là anh/chị. Giọng tự nhiên như Zalo thật, ngắn gọn 2-4 câu, có quan điểm cá nhân. Khi viết kịch bản phải giữ placeholder <Xưng hô> <Tên Zalo người nhận> để tái dùng, và có thể chèn ảnh.' WHERE id = ?`, [def.id]);
              Logger.log('[DatabaseService] Migrated default AI assistant to gpt-5.6-luna + dynamic prompt');
            }
          } else {
            // Chưa có default -> tạo mới
            const id = 'adn-default-' + Date.now();
            const now = Date.now();
            this.run(`INSERT INTO ai_assistants (id, name, platform, api_key_encrypted, model, system_prompt, max_tokens, temperature, context_message_count, enabled, is_default, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [id, 'ADN Assistant - Huy', 'openai', '', 'gpt-5.6-luna', 'Bạn là {{senderName}}, nhân viên tư vấn của ADN Capital (người chính là Huy, các nick còn lại là clone của Huy). Xưng hô theo tên này, gọi khách là anh/chị. Giọng tự nhiên như Zalo thật, ngắn gọn 2-4 câu, có quan điểm cá nhân. Khi viết kịch bản phải giữ placeholder <Xưng hô> <Tên Zalo người nhận> để tái dùng, và có thể chèn ảnh.', 800, 0.92, 30, 1, 1, now, now]);
            Logger.log('[DatabaseService] Seeded default ADNzalo assistant gpt-5.6-luna');
          }
        } catch (e: any) { Logger.warn(`[DatabaseService] default assistant migration: ${e.message}`); }
     ...
     *
     * Example:
     *   baseDir = "C:\Users\Admin\AppData\Roaming\ADNzalo\media"
