...
const DEFAULT_DB_NAME = 'adnzalo-tool.db';           // existing DB
...
class WorkspaceManager {
     ...
     * Existing adnzalo-tool.db stays in place - the default workspace simply points to it.
     */
    private migrateFromLegacy(): void {
        ...
        const adnzaloConfigPath = path.join(this.userDataPath, 'adnzalo-config.json');
        if (fs.existsSync(adnzaloConfigPath)) {
            try {
                const cfg = JSON.parse(fs.readFileSync(adnzaloConfigPath, 'utf-8'));
    ...
    public createWorkspace(params: {
        ...
        //   workspace-{id}/adnzalo-tool.db + workspace-{id}/media/
        const wsFolder = `workspace-${id}`;
        const wsDbRelative = `${wsFolder}/adnzalo-tool.db`;
    ...
    public deleteWorkspace(id: string): { success: boolean; error?: string } {
        ...
        const wsDbPath = ws.dbPath || `workspace-${id}/adnzalo-tool.db`;
        ...
        // SAFETY: Never delete the root adnzalo-tool.db (belongs to default workspace)
     ...
     * Respects custom dbFolder from adnzalo-config.json.
     */
