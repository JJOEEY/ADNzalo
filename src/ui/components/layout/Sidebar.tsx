...
export default function Sidebar({ onAddAccount }: SidebarProps) {
  ...
  // ADNzalo: thân thiện hơn - mặc định mở rộng để thấy label
  const [adnExpanded, setAdnExpanded] = useState(() => {
    try { return localStorage.getItem('adn_sidebar_expanded') !== 'false'; } catch { return true; }
  });
  useEffect(() => { try { localStorage.setItem('adn_sidebar_expanded', String(adnExpanded)); } catch {} }, [adnExpanded]);
  const isExpanded = adnExpanded;
  ...
  return (
    <div className={`flex flex-col bg-gray-900 border-r border-gray-700 h-full transition-all duration-200 ${isExpanded ? 'w-64' : 'w-16'}`}>
      ...
      <div className="h-14 flex items-center gap-2 px-3 border-b border-gray-700/50 flex-shrink-0">
        ...
        {isExpanded && (
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-gray-400 truncate">Zalo CRM • {accounts.length} nick</p>
          </div>
        )}
        <button
          onClick={() => setAdnExpanded(!isExpanded)}
          title={isExpanded ? 'Thu gọn' : 'Mở rộng'}
          className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-700 transition-colors flex-shrink-0"
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            {isExpanded ? <><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></> : <><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="18" x2="20" y2="18"/></>}
          </svg>
        </button>
      </div>
      {/* Toggle cũ cho Chat - giữ để không vỡ logic, ẩn đi khi đã có nút trên */}
      {false && view === 'chat' && (
      ...
      {/* Nav - thân thiện: có label khi mở rộng */}
...
function NavBtnFriendly({ icon, label, desc, active, onClick, dot, expanded }: { icon: string; label: string; desc?: string; active: boolean; onClick: () => void; dot?: boolean; expanded?: boolean }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    crm: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M7 14v3"/><path d="M12 10v7"/><path d="M17 7v10"/></svg>,
    workflow: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="5" cy="6" r="2"/><circle cx="19" cy="6" r="2"/><circle cx="12" cy="18" r="2"/><path d="M7 6h10M5 8v4a7 7 0 0 0 7 7M19 8v4a7 7 0 0 1-7 7"/></svg>,
    integration: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>,
  ...
  const base = `flex items-center gap-3 rounded-xl transition-colors ${expanded ? 'w-full px-3 py-2.5' : 'w-10 h-10 justify-center'}`;
  const activeCls = active ? 'bg-blue-600 text-white shadow' : 'text-gray-400 hover:bg-gray-800 hover:text-white';
  return (
    <button onClick={onClick} title={label} className={`${base} ${activeCls} relative`}>
      <span className="flex-shrink-0">{icons[icon]}</span>
      {expanded && <span className="flex-1 text-left min-w-0"><span className="block text-xs font-semibold leading-none truncate">{label}</span>{desc && <span className="block text-[10px] opacity-70 truncate">{desc}</span>}</span>}
      {dot && <span className={`bg-red-500 rounded-full ${expanded ? 'w-2 h-2 ml-auto' : 'absolute top-1 right-1 w-2 h-2 border border-gray-900'}`} />}
    </button>
  );
}
...
const TOOLS_GUIDE = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          'Nhãn Zalo (Zalo Label): đồng bộ 2 chiều với app Zalo trên điện thoại - gán từ ADNzalo, thấy trên Zalo và ngược lại',
          'Nhãn Local: nhãn riêng của ADNzalo, tùy biến màu sắc + emoji, không giới hạn số lượng',
