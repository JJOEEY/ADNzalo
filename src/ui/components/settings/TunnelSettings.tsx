  ...
  const services: WebhookService[] = [
    {
      key: 'integration',
      port: intPort,
      label: 'Webhook bên ngoài',
      icon: <LinkIcon className="w-4 h-4" />,
      description: 'Nhận dữ liệu webhook từ dịch vụ bên ngoài để dùng trong workflow.',
      uses: [
        'Nhận callback hoặc sự kiện từ website và ứng dụng bên ngoài',
        'Kích hoạt các workflow có trigger webhook',
      ],
      guideTitle: 'Hướng dẫn: Nhận webhook từ dịch vụ bên ngoài',
      guideSteps: [
        'Bật tunnel để tạo URL công khai cho ứng dụng đang chạy trên máy',
        'Tạo workflow với trigger "Webhook bên ngoài"',
        'Sao chép URL webhook trong cấu hình workflow sang dịch vụ gửi dữ liệu',
        'Khi dịch vụ gửi dữ liệu, workflow sẽ bắt đầu xử lý',
      ],
      showPortConfig: true,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl p-4 space-y-3 border border-amber-600 dark:border-0">
        ...
        <p className="text-xs dark:text-gray-400 leading-relaxed">
          ADNzalo dùng <strong className="text-blue-600 dark:text-blue-300">Cloudflare Quick Tunnel</strong> (miễn phí, không cần tài khoản)
          để tạo các URL này. Mỗi nhóm webhook có một URL riêng.
        </p>
        <div className="border rounded-lg px-3 py-2 space-y-1.5">
          <p className="text-xs font-medium "><SunIcon className="w-4 h-4 inline" /> Nếu bạn chỉ dùng ADNzalo trong mạng LAN (cùng WiFi) thì <strong>không cần bật tunnel</strong>.
