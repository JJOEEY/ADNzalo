...
export default function TargetSelector({ zaloId, allLabels, localLabels, localLabelThreadMap, existingContactIds, onConfirm, onClose, headerContent }: TargetSelectorProps) {
  ...
  useEffect(() => {
    ...
    // Tách theo dòng, mỗi dòng trích SĐT bằng regex có hỗ trợ khoảng trắng/dấu trong số
    const lines = phoneInput.split(/[\n,;]+/);
    const rawCandidates: string[] = [];
    const phoneRegex = /(?:\+84|84|0)[\s\-\.\(\)]*\d(?:[\s\-\.\(\)]*\d){7,11}\b/g;
    for (const line of lines) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      const matches = trimmed.match(phoneRegex);
      if (matches && matches.length > 0) rawCandidates.push(...matches);
      else rawCandidates.push(trimmed);
    }
    const normalized = [...new Set(rawCandidates.map(normalizePhone).filter(s => /^0[35789]\d{8}$/.test(s)))];
