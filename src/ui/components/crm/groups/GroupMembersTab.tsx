...
interface ScanAccountCreds { zaloId: string; cookies: string; imei: string; userAgent: string }

/**
 * Quét 1 nhóm qua backend — xoay vòng tối đa 4 nick Zalo.
 * Nick chính quét trước; nếu bị lockViewMember giới hạn (≤10 người) thì thử
 * các nick khác (nick nào là admin/phó nhóm của nhóm đó sẽ lấy được đầy đủ),
 * giữ kết quả nhiều nhất.
 */
async function scanGroupViaBackendWithRotation(creds: ScanAccountCreds[], primaryZaloId: string, groupId: string): Promise<any> {
  const { scanGroupViaBackend } = await import('@/lib/backendService');
  const ordered = [...creds].sort((a, b) => Number(b.zaloId === primaryZaloId) - Number(a.zaloId === primaryZaloId));
  let best: any = null;
  let lastError = 'Quét thất bại';
  for (const acc of ordered.slice(0, 4)) {
    if (!acc.zaloId || !acc.cookies || !acc.imei) continue;
    try {
      const res = await scanGroupViaBackend({
        pageId: acc.zaloId,
        cookie: acc.cookies,
        imei: acc.imei,
      ...
      const count = res?.success ? (res.members?.length || 0) : 0;
      if (count > (best?.members?.length || 0)) best = res;
      // Đã lấy được danh sách đông (không còn dấu hiệu bị giới hạn) → dừng sớm
      if (best && best.members.length > 10) break;
  ...
  return best || { success: false, groupId, totalMembers: 0, members: [], error: lastError };
}
...
function ZaloGroupMembersTab() {
  ...
  // ── Bulk scan all groups state (Phase C) ──────────────────────────────────
  const [bulkScanning, setBulkScanning] = useState(false);
  const [bulkCurrent, setBulkCurrent] = useState<{ index: number; total: number; groupId: string; groupName: string } | null>(null);
  const [bulkErrors, setBulkErrors] = useState<Array<{ groupId: string; name: string; error: string }>>([]);
  const [bulkProgress, setBulkProgress] = useState<{ scanned: number; saved: number; failed: number } | null>(null);
  ...
  const handleScanTab = useCallback(async () => {
    ...
    try {
      ...
      // Quét qua backend — xoay vòng các nick Zalo khi nick chính bị giới hạn quyền xem
      const allZaloAccs = useAccountStore.getState().accounts.filter((a: any) => (a.channel || 'zalo') === 'zalo');
      const creds: ScanAccountCreds[] = allZaloAccs.map((a: any) => ({
        zaloId: a.zalo_id,
        cookies: a.cookies || '',
        imei: a.imei || '',
        userAgent: a.user_agent || '',
      }));
      const result = await scanGroupViaBackendWithRotation(creds, activeAccountId, groupId);

      if (!result?.success) {
        // Thử vét DOM qua cửa sổ web Zalo khi backend báo lỗi/limit
        try {
          const scrapeRes: any = await (ipc as any).zalo?.scrapeGroupMembers({ auth, groupId, accountKey: activeAccountId });
          if (scrapeRes?.needsWebLogin) {
            setScanTabError('Cần đăng nhập Zalo Web một lần cho nick này: cửa sổ đã mở — quét mã QR rồi nhấn Quét lại.');
            return;
          }
      ...
      // Nếu backend chỉ trả <=10 trong khi group đông, thử vét DOM qua cửa sổ web
  ...
  // ── Bulk scan all groups (Phase C - Active hidden-member scan per-account) ──
  const handleScanAllGroups = useCallback(async () => {
    if (!activeAccountId || groups.length === 0) return;
    const acc = useAccountStore.getState().getActiveAccount();
    if (!acc) return;
    bulkStopRef.current = false;
    setBulkScanning(true);
    setBulkErrors([]);
    setBulkProgress({ scanned: 0, saved: 0, failed: 0 });
    setBulkCurrent(null);
    let saved = 0;
    let failed = 0;
    ...
    try {
      ...
      for (let i = 0; i < groups.length; i++) {
        ...
        const g = groups[i];
        setBulkCurrent({ index: i + 1, total: groups.length, groupId: g.contact_id, groupName: g.display_name });
        try {
          const result = await scanGroupViaBackendWithRotation(creds, activeAccountId, g.contact_id);
          if (result?.success && result.members && result.members.length > 0) {
            // Nếu backend chỉ trả <=10 trong khi group đông thì thử vét DOM qua cửa sổ web
            const isLimited = result.members.length <= 10 && (result.totalMembers || 0) > result.members.length;
            if (isLimited) {
              try {
                const auth = buildZaloAuth(acc, activeAccountId);
                const scrapeRes: any = await (ipc as any).zalo?.scrapeGroupMembers({ auth, groupId: g.contact_id, accountKey: activeAccountId });
            ...
            try {
              ...
              if ((check?.members?.length ?? 0) > 2) fallbackSaved = true; // >2 để loại trừ chỉ có trưởng/phó
                ...
                if (linkScan) fallbackSaved = true;
                  ...
                  // DOM scraper qua cửa sổ web Zalo cho lockViewMember
                  try {
                    const scrapeRes: any = await (ipc as any).zalo?.scrapeGroupMembers({ auth, groupId: g.contact_id, accountKey: activeAccountId });
                    if (scrapeRes?.success && Array.isArray(scrapeRes.members) && scrapeRes.members.length > 2) {
        ...
        setBulkProgress({ scanned: i + 1, saved, failed });
        if (!bulkStopRef.current && i + 1 < groups.length) {
  ...
  }, [activeAccountId, groups, selectedGroupId, loadGroupsFromDB, loadMembersFromDB]);

  // ── Premium status ────────────────────────────────────────────────────
  // ADNzalo: quét ẩn luôn miễn phí, không còn gói Premium
  const loadPremiumStatus = useCallback(async (_fromBackend = false) => {
    if (!activeAccountId) return;
    setIsPremium(true);
  ...
  useEffect(() => {
    ...
    // Reset bulk scan state per-account
    ...
    <div className="flex flex-col flex-1 overflow-hidden relative">
      ...
      <div className="flex items-center border-b border-gray-700 flex-shrink-0 bg-gray-900/50">
        ...
        <button
          ...
          Quét thành viên
          <span className="ml-1 px-1.5 py-0.5 text-[10px] font-bold bg-green-500/20 text-green-400 rounded-full leading-none">Miễn phí</span>
          {!scanTabSeen && (
          ...
          <div className="mx-auto px-6 py-6 space-y-5 relative">
            ...
            <div className="text-center pb-2 relative">
              ...
              <div className="absolute top-0 right-0 flex items-center gap-2 z-10">
                <button
                  onClick={() => ipc.shell?.openExternal('https://adncapital.com.vn')}
            ...
            {/* ADNzalo: Tự động nhận diện nhóm của tài khoản - không cần link */}
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold text-white">Nhóm của tài khoản đang chọn <span className="text-blue-400">({groups.length} nhóm)</span></p>
                <div className="flex gap-2">
                  <button onClick={fetchGroupsFromAPI} disabled={groupsLoading || bulkScanning} className="px-3 py-1.5 text-xs bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-lg border border-gray-600 disabled:opacity-50">Làm mới</button>
                  <button
                    onClick={() => { setScanTabSeen(true); try{localStorage.setItem('scanTabSeen','true')}catch{}; handleScanAllGroups(); }}
                    disabled={bulkScanning || groups.length===0}
                    className="px-3 py-1.5 text-xs bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50 flex items-center gap-1.5">
                    {bulkScanning && bulkCurrent ? <>{SpinIcon} Đang quét {bulkCurrent.index}/{bulkCurrent.total} — {bulkCurrent.groupName.slice(0,18)}</> : <>Quét tất cả {groups.length} nhóm</>}
                  </button>
                  {bulkScanning && (
                    <button onClick={() => { bulkStopRef.current = true; }} className="px-3 py-1.5 text-xs bg-red-600 hover:bg-red-700 text-white rounded-lg flex items-center gap-1">
              ...
              {/* Bulk scan inline progress */}
              {bulkCurrent && (
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-blue-300 flex items-center gap-1.5">{SpinIcon} Đang quét {bulkCurrent.index}/{bulkCurrent.total} — {bulkCurrent.groupName}</span>
                    <span className="text-gray-400 font-medium">{bulkCurrent.index}/{bulkCurrent.total}</span>
                  </div>
                  <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full transition-all duration-300" style={{ width: `${(bulkCurrent.index / bulkCurrent.total) * 100}%` }} />
                  </div>
                  {bulkProgress && <p className="text-[11px] text-gray-400">Đã quét: {bulkProgress.scanned} · Thành công: <span className="text-green-400">{bulkProgress.saved}</span> · Lỗi: <span className="text-red-400">{bulkProgress.failed}</span></p>}
                </div>
              )}
              {bulkProgress && !bulkScanning && (
                <div className="px-3 py-2 bg-green-500/10 border border-green-500/30 rounded-lg text-xs text-green-400 flex items-center gap-1.5">
                  <CheckIcon className="w-3.5 h-3.5" /> Hoàn tất: {bulkProgress.scanned}/{groups.length} nhóm · Thành công: {bulkProgress.saved} · Lỗi: {bulkProgress.failed}
                </div>
              )}
              {bulkErrors.length > 0 && (
                <div className="border border-red-500/30 rounded-lg divide-y divide-red-500/20 max-h-32 overflow-y-auto">
                  <p className="px-3 py-2 text-xs font-medium text-red-400 bg-red-500/10">Lỗi {bulkErrors.length} nhóm:</p>
                  {bulkErrors.map(e => (
                    <div key={e.groupId} className="px-3 py-1.5 text-[11px] text-gray-300 flex justify-between gap-2">
                      <span className="truncate flex-1">{e.name} ({e.groupId})</span>
                      <span className="text-red-400 truncate max-w-[160px]">{e.error}</span>
                    </div>
                  ))}
                </div>
              )}
              {groups.length > 0 ? (
                <div className="max-h-40 overflow-y-auto border border-gray-700 rounded-lg divide-y divide-gray-700/50">
                  {groups.slice(0, 20).map(g => (
                    <div key={g.contact_id} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-700/30">
                      <img src={g.avatar_url ? toLocalMediaUrl(g.avatar_url) : ''} alt="" className="w-6 h-6 rounded-full bg-gray-600 flex-shrink-0" onError={e => (e.target as HTMLImageElement).style.display='none'} />
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-white truncate">{g.display_name}</p>
                        <p className="text-[10px] text-gray-400">{g.memberCount ? `${g.memberCount} thành viên` : g.contact_id}</p>
                      </div>
                      <button onClick={() => { setScanLinkInput(g.contact_id); setTimeout(()=>handleScanTab(), 100); }} disabled={bulkScanning} className="px-2 py-1 text-[11px] bg-purple-600 hover:bg-purple-700 text-white rounded disabled:opacity-50">Quét</button>
                    </div>
                  ))}
                  {groups.length > 20 && <p className="text-[11px] text-gray-400 text-center py-1">+ {groups.length - 20} nhóm nữa...</p>}
                </div>
              ) : (
                <p className="text-xs text-gray-400 text-center py-2">Chưa có nhóm nào - bấm Làm mới để đồng bộ từ Zalo</p>
              )}
              <p className="text-[11px] text-gray-500">Tự động liệt kê nhóm của nick đang chọn, không cần dán link. Bấm Quét để lấy thành viên ẩn.</p>
            </div>

            {/* Scan input (vẫn giữ cho trường hợp có link) */}
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-3">
              <p className="text-xs text-gray-400">Hoặc dán link nhóm nếu chưa tham gia:</p>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={scanLinkInput}
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-white font-medium">Quét thành viên ẩn</span>
                </div>
                <span className="px-2 py-0.5 bg-green-500/15 text-green-400 text-[11px] font-medium rounded-full">Miễn phí</span>
              </div>
              <p className="text-xs text-gray-400 leading-relaxed">
                Quét không giới hạn số nhóm và số thành viên — hệ thống tự xoay vòng các nick đã đăng nhập để lấy danh sách đầy đủ nhất, và tích lũy dần qua từng lần quét.
              </p>
            </div>
 ...
 * above contains scan flows that have no Telegram API equivalent, so none of
 * those controls leak into this branch. */
